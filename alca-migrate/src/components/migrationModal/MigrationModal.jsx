...
import { Button, Icon, Modal, Message, Popup } from "semantic-ui-react";
...
export function MigrationModal({ migrationAmount, isOpen, successAction, closeModal }) {
    ...
    const { madAllowance, alcaExchangeRate } = useSelector((state) => ({
    ...
    const AllowStep = () => (
        <div>
            Click the button for <b>Step 1</b> below to begin the migration process
            <br />
    ...
    const MigrateStep = () => (
        <div>
            Click the button for <b>Step 2</b> below to complete the migration process
            <br />
            ...
            <Popup
                ...
                trigger={
                    <div className="text-sm">
                        ...
                        {latestTxHash && <Icon name="external" className="m-0 h-full absolute right-3 top-2.5" />}
                    </div>
                }
            />
        </div>
    ...
    return (
        <Modal
            closeIcon={!migrationSuccess && !waiting}
            size="small"
            open={isOpen}
            closeOnDimmerClick={!migrationSuccess && !waiting}
            onClose={() => {
                closeModal();
                setError(false);
                setSuccess(false);
