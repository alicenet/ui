...
export function StakeActions() {
    ...
    const getActiveTab = () => {
        switch (activeItem) {
            case "welcome":
                return <StakeWelcome stepForward={() => setActiveItem("stake")} />;
            case "stake":
                return <StakeStake />;
            case "unstake":
                return <StakeUnstake />;
                ...
                <Grid padded className="flex h-full">
                    <Grid.Row>
                        <Grid.Column width={3}>
                            <Menu fluid vertical tabular>
                                ...
                                <Menu.Item
                                    content={
                                        <>
                                            ...
                                            <div className="text-xs">{formatNumberToLocale(alcaBalance)} ALCA Available</div>
                                        </>
                                    }
                                    disabled={!hasReadTerms}
                                ...
                                <Menu.Item
                                    content={
                                        <>
                                            ...
                                            <div className="text-xs">{stakedAlca > 0 ? "" : "No ALCA staked"}</div>
                                        </>
                                    }
                                ...
                                <Menu.Item
                                    content={
                                        <>
                                            <Header className={classNames({ "opacity-40": !hasReadTerms || alcaRewards == 0 || ethRewards == 0 })}>
                                                Claim
                                            </Header>
                                            <div className="text-xs">{ethRewards > 0 ? "" : "No ETH to claim"}</div>
                                            <div className="text-xs">{alcaRewards > 0 ? "" : "No ALCA to claim"}</div>
                                        </>
                                    }
                        ...
                        <Grid.Column stretched width={13}>
                            <Segment disabled={!web3Connected}>{getActiveTab()}</Segment>
                        </Grid.Column>
                    </Grid.Row>
                </Grid>
            </div>
